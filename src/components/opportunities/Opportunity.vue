  ...
  <div class="wrapper">
    ...
    <Row class="row">
      ...
      <i-col span="4" style="text-align:right;padding-right:10px">
        <Button icon="ios-add" type="primary" size="small" @click="clickAdd">新增营销事件</Button>
      </i-col>
    </Row>
    <Row>
      <i-col span="24">
      ...
      <Form ref="Data_" :label-width="110" :rules="rules" :model="Data_">
        ...
        <Row>
          <i-col span="12">
            <FormItem label="原始事件名称" prop="oreventId">
              <Select v-model="Data_.oreventId" placeholder="请选择原始事件名称" clearable :disabled="!editMode" @on-change="originalEventChange">
                <Option v-for="(item,key) in events" :key="key" :value="key">{{item}}</Option>
              </Select>
            </FormItem>
          </i-col>
        ...
        <Collapse v-model="conditions" simple>
          <Panel v-for="(item,index) in commonCondition" :key="index" :name="item.labelType">
            <b>{{item.labelType}}</b>
            <div slot="content">
              <Row v-for="(condition,index) in item.data" :key="index"  style="padding-bottom:10px">
                <i-col span="5" style="line-height:25px;text-align:center;">{{condition.labelName}}</i-col>
                <i-col span="19" style="line-height:25px">
...
export default {
  ...
  data () {
    return {
      ...
      eventDimenions: [],
      // 表达式类型选项
      expTypes: {'10': '等于', '11': '不等于', '12': '大于', '13': '小于', '14': '大于等于', '15': '小于等于', '16': '包含', '17': '不包含'},
      // 营销事件类型选项
      marketingTypes: [],
      // 事件维度列表栏目定义
      dimensionCols: [
  ...
  methods: {
    ...
    clickAdd () {
      if (this.editMode && this.currentRow >= 0) {
        this.$Modal.confirm({
          title: '确认放弃保存',
          content: '您修改的营销事件数据还未保存，您确认要放弃保存而去新建营销事件吗？',
          onOk: () => {
            this.modalTitle = '新建营销事件'
            this.currentRow = -1
            this.showModal = true
          }
        })
    ...
    // 原始事件改变时触发
    originalEventChange (eventId) {
      this.getDimensionNames(eventId)
      // this.Data_.filters = [{dimensionName: '', expressType: '', expressValue: ''}]
    },
    // 获取当前页列表数据
    getData () {
    ...
    saveRow () {
      this.$httpReq('/marketingEvent/' + this.Data_.id, {rowData: this.Data_}, 'edit', (res) => {
        this.$Message.success('修改后营销事件数据已成功保存！')
        this.getData()
      })
    },
    // 删除数据，点击“删除”按钮时触发
    delRow (rowData) {
      this.$httpReq('/marketingEvent/' + rowData.id, '', 'del', (res) => {
        this.$Message.success('所选营销事件已成功删除！')
        this.getData()
      })
    },
    // 新增数据，点击模态对话框“保存”按钮时触发
    addRow () {
      this.$httpReq('/marketingEvent/0', {rowData: this.Data_}, 'add', (res) => {
        this.$Message.success('营销事件已成功新增！')
        this.getData()
        this.newRow.chanceName = ''
        this.newRow.chanceType = null
        this.newRow.eventId = null
        this.newRow.oreventId = null
        this.newRow.priority = 1
        this.newRow.limitDays = []
        this.newRow.limitCycle = 0
        this.newRow.limitUnit = null
        this.newRow.remark = ''
        this.newRow.filters = []
        this.newRow.settings = {}
      })
    },
    handleCheckAll () {
      if (!this.editMode) {
    ...
    // 设置营销事件开关状态
    setStatus (rowData) {
      this.$httpReq('/marketingEvent/setStatus/' + rowData.id, {}, 'edit', (res) => {
        this.$Message.success('营销事件开关状态修改成功')
        this.getData()
      })
    }
  }
